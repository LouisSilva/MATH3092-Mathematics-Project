...
class AudioTestCase(ABC):
    ...
    @abstractmethod
    def apply(self, x_Q: np.ndarray, f_s: int, rng: np.random.Generator) -> np.ndarray:
...
class WhiteNoiseTest(AudioTestCase):
    ...
    def apply(self, x_Q: np.ndarray, f_s: int, rng: np.random.Generator) -> np.ndarray:
        P_Q = np.mean(x_Q ** 2) + 1e-12
        P_omega = P_Q / (10 ** (self.params['snr_db'] / 10))

        omega_standard_deviation = np.sqrt(P_omega)
        omega = rng.normal(loc=0, scale=omega_standard_deviation, size=x_Q.shape[0]).astype(x_Q.dtype)
...
class PitchShiftTest(AudioTestCase):
    ...
    def apply(self, x_Q: np.ndarray, f_s: int, rng: np.random.Generator) -> np.ndarray:
        return PitchShift(semitones=self.params['n_steps'])(x_Q, f_s)
...
class ReverbTest(AudioTestCase):
    ...
    def apply(self, x_Q: np.ndarray, f_s: int, rng: np.random.Generator) -> np.ndarray:
        return Reverb(room_size=self.params['room_size'],
                      ...
                      dry_level=self.params['dry_level'])(x_Q, f_s)
...
class DistortionTest(AudioTestCase):
    ...
    def apply(self, x_Q: np.ndarray, f_s: int, rng: np.random.Generator) -> np.ndarray:
        return Distortion(drive_db=self.params['drive_db'])(x_Q, f_s)
...
class ClippingDbTest(AudioTestCase):
    ...
    def apply(self, x_Q: np.ndarray, f_s: int, rng: np.random.Generator) -> np.ndarray:
        return Clipping(threshold_db=self.params['threshold_db'])(x_Q, f_s)
